...
struct Node1
{
	...
	Node1(pcl::PointXYZI arr, int setId)
		: point(arr), id(setId), left(NULL), right(NULL)
	{
	}
};
...
struct KdTree
{
	...
	void insertHelper3d(Node1 **node, uint depth, pcl::PointXYZI point, int id)
	{
		//Tree is empty
		if (*node == NULL)
	...
	void searchHelper3d(pcl::PointXYZI target, Node1 *node, int depth, float distanceTol, std::vector<int> &ids)
	{
		if (node != NULL)
		{
